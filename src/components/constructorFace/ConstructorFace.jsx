...
import WOMEN_FACE from "../../assets/imgs/const-face.png";
import { useState, useEffect } from "react";

const stroke = "#000";
const strokeOpacity = 0.5;
const strokeDasharray = 3;
const svgClassNames = "sgv-line";
const pathClassNames = "svg-line-path";

const ConstructorFace = ({ activeLine, setActiveLine }) => {
  ...
  useEffect(() => {
    if (activeLine) {
      console.log("Выбранная часть:", activeLine);
    }
  }, [activeLine]);
  ...
  const getLineClassName = (lineId) => {
    return `constructor-face__line ${lineId} ${
      activeLine === lineId ? "active" : ""
    }`;
  };

  const getGroupClassName = (lineId) => {
    return `constructor-face__group ${lineId} ${
      activeLine === lineId ? "active" : ""
    }`;
  };
  ...
  return (
    <div className="constructor-face__container">
      <div className="constructor-face__wrapper">
        <img src={WOMEN_FACE} className="constructor-face__img" alt="face" />
        <div className="constructor-face__lines">
          <div
            className={getLineClassName("forehead")}
            onClick={() => handleMouseEnter("forehead")}
          >
            {lineForehead}
          </div>
          <div
            className={getGroupClassName("eyes")}
            onClick={() => handleMouseEnter("eyes")}
          >
            <div className="constructor-face__line eyes left">{lineEyes}</div>
            <div className="constructor-face__line eyes right">{lineEyes}</div>
          </div>
          <div
            className={getGroupClassName("cheekbones")}
            onClick={() => handleMouseEnter("cheekbones")}
          >
            <div className="constructor-face__line cheekbones left">
              {lineCheekbones}
            </div>
            <div className="constructor-face__line cheekbones right">
              {lineCheekbones}
            </div>
          </div>
          <div
            className={getLineClassName("nose")}
            onClick={() => handleMouseEnter("nose")}
          >
            {lineNose}
          </div>
          <div
            className={getLineClassName("lips")}
            onClick={() => handleMouseEnter("lips")}
          >
            {lineLips}
          </div>
          <div
            className={getLineClassName("ears")}
            onClick={() => handleMouseEnter("ears")}
          >
            {lineEars}
          </div>
        </div>
